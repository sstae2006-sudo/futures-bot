...
| `src/futures_bot/context/` | Market Context Engine (in progress, 2026-07-27) — `MarketContext` value object, `ContextEngine`, `session.py`'s `SessionContext`/`classify_session` and `volatility.py`'s `VolatilityContext`/`analyze_volatility` (both real; the other four classification dimensions are still stubs). Not wired into `TradingEngine`/`Strategy` yet. See section 8 and `docs/ARCHITECTURE.md`'s "Market Context Engine" section. |
...
wired in yet; session and volatility classification are real, the
other four dimensions are still stubs — see `docs/ARCHITECTURE.md`'s "Market
