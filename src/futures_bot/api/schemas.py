...
class MaintenanceStatusOut(BaseModel):
    ...
    counted for a human to act on. `stale_workers_marked_offline_count`
    (SIL Phase 6 Milestone 2) IS cumulative, like `stale_drafts_discarded_count`
    -- unlike an orphaned org, marking a worker `offline` is reversible
    (its next heartbeat brings it back `online`), so this is a real,
    ongoing write path, not merely a count for a human to review."""
    running: bool
    last_cycle_at: Optional[str] = None
    last_result: Optional[str] = None
    last_error: Optional[str] = None
...
class WorkItemReviewOut(BaseModel):
    ...
    see `docs/ARCHITECTURE.md`'s "SIL Phase 6" section. Live and
    ephemeral (never persisted) -- see `IntegrationReviewOut` below for
    Milestone 2's permanent historical counterpart."""
    work_item: WorkItemOut
    merge_readiness: MergeReadinessOut
    readiness_note: Optional[str] = None
...
class ValidationRecommendationOut(BaseModel):
    """SIL Phase 6 Milestone 2's "Validation Planning" -- reuses
    `tools/local_validate.py`'s own file-to-test heuristic
    (`collaboration/validation_planning.py`) rather than a second,
    independently-drifting mapping. `recommend_full_suite=True` means at
    least one changed file had no direct test mapping -- "never silently
    under-test," matching that tool's own default behavior."""
    recommended_tests: list[str] = Field(default_factory=list)
    unmapped_files: list[str] = Field(default_factory=list)
    recommend_full_suite: bool = False
    frontend_validation_recommended: bool = False


class ConflictResolutionOut(BaseModel):
    """One Overlap Engine V2 warning, enriched with the Conflict
    Resolution Assistant's (`collaboration/conflict_resolution.py`)
    subsystem impact and a templated suggested resolution/integration
    order -- never an automated merge action, only advice for a human or
    AI worker to act on."""
    work_item_id: str
    title: str
    risk: RiskLevelLiteral
...
class GenerateIntegrationReviewRequest(BaseModel):
    """`worker_id` is optional -- a review can be generated by a human
    triggering it manually (no worker row involved) as well as by a
    registered worker."""
    worker_id: Optional[str] = None


class IntegrationReviewOut(BaseModel):
    """One PERMANENT Integration Review row -- `POST`/`GET
    /api/work-items/{item_id}/reviews`. Unlike every other primitive in
    `collaboration/` (merge_readiness, overlap_v2, context_bundle, worker
    staleness -- all "compute live, never persist"), this is a deliberate
    exception: the point is a historical record of what the coordinator
    concluded at a given moment, so confidence trends are visible over
    time. Never overwritten; a new review is always a new row (see
    `collaboration/store.py::create_integration_review`'s docstring).
    Built entirely from existing primitives (merge readiness, Overlap
    Engine V2, the Conflict Resolution Assistant, a minimal subsystem
    lookup, and the shared validation-planning heuristic) -- no new
    scoring model."""
    id: str
    work_item_id: str
    worker_id: Optional[str] = None
    branch: Optional[str] = None
    status_at_review: WorkItemStatusLiteral
    confidence_score: int = Field(ge=0, le=100)
    risk_level: RiskLevelLiteral
    level: MergeReadinessLevelLiteral
    related_work_item_ids: list[str] = Field(default_factory=list)
    affected_subsystems: list[str] = Field(default_factory=list)
    conflict_resolutions: list[ConflictResolutionOut] = Field(default_factory=list)
    validation_recommendation: ValidationRecommendationOut
    readiness_note: Optional[str] = None
    summary: str
    recommendation: str
    created_at: str
